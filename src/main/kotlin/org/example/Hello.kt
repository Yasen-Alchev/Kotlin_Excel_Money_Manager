...
fun Sheet.createItem(item: Item){
...
fun findTodaysRow(sheet: Sheet): Int
{
...
fun findItemCellIndex(sheet: Sheet, item: String): Int
{
...
fun writeToFile(filename: String, item: Item)
{
    ...
    if(file.exists())
    {
        ...
        val sheet = xlWb.getSheet("Razhodi")
        val row = findTodaysRow(sheet)
        val cell = findItemCellIndex(xlWb.getSheet("Razhodi"), item.name.toString())
        ...
        xlWb.write(FileOutputStream(filename))
        xlWb.close()
    }
    else
    {
        ...
        try {
            when(userinput.toInt()) {
                1 -> {
                    ...
                    if (itemPrice != null) {
                        writeToFile(fileName, Item(itemName, itemPrice))
                    }
                    else{
                        println("Invalid price")
                    }
                }
                2 -> readFromFile(fileName)
                else -> {
                    println("\tWrong input")
                    println("\tWrite to file: 1")
                    println("\tRead from file: 2")
                }
            }
        }
        catch (e: Exception)
        {
            println("Error Entering number")
            println(e)
        }
        print("\nInput: ")
